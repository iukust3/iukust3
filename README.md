<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://capsule-render.vercel.app/api?type=waving&color=0:0D1117,25:1E1F3B,55:6C5CE7,80:8E44AD,100:00CEC9&height=230&section=header&text=Irfan%20Ullah&fontSize=52&fontColor=ffffff&fontAlignY=35&desc=Project%20Manager%20%7C%20Principal%20KMP%20Architect%20%7C%2012%2B%20Years%20Exp&descAlignY=58&descSize=16&descColor=dfe6e9&animation=fadeIn">
    <source media="(prefers-color-scheme: light)" srcset="https://capsule-render.vercel.app/api?type=waving&color=0:F0EEFF,25:DDD6FE,55:C4B5FD,80:A78BFA,100:8B5CF6&height=230&section=header&text=Irfan%20Ullah&fontSize=52&fontColor=1B1F3B&fontAlignY=35&desc=Project%20Manager%20%7C%20Principal%20KMP%20Architect%20%7C%2012%2B%20Years%20Exp&descAlignY=58&descSize=16&descColor=3D3D5C&animation=fadeIn">
    <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0D1117,25:1E1F3B,55:6C5CE7,80:8E44AD,100:00CEC9&height=230&section=header&text=Irfan%20Ullah&fontSize=52&fontColor=ffffff&fontAlignY=35&desc=Project%20Manager%20%7C%20Principal%20KMP%20Architect%20%7C%2012%2B%20Years%20Exp&descAlignY=58&descSize=16&descColor=dfe6e9&animation=fadeIn" width="100%" alt="Header Banner"/>
  </picture>

  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=21&pause=1200&color=6C5CE7&center=true&vCenter=true&width=880&lines=🚀+Project+Manager+%26+Lead+Kotlin+Multiplatform+Architect;📱+Kotlin+Multiplatform+%7C+Compose+Multiplatform+%7C+iOS+%26+Android;⚡+High-Concurrency+Ktor+%26+Node.js+Distributed+Microservices;📍+Real-Time+OSRM+Routing%2C+Geofencing+%26+Telemetry+Engines;🛠️+12%2B+Years+Transforming+Complex+Challenges+Into+Scalable+Products" alt="Typing SVG" />
  </a>

  <p align="center">
    <a href="https://www.linkedin.com/in/irfanullah2/" target="_blank">
      <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
    </a>
    <a href="mailto:itsme.irfanullah@gmail.com">
      <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/>
    </a>
    <a href="https://github.com/iukust3">
      <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
    </a>
    <a href="https://komarev.com/ghpvc/?username=iukust3">
      <img src="https://komarev.com/ghpvc/?username=iukust3&label=Profile%20Views&color=6C5CE7&style=for-the-badge" alt="Profile Views"/>
    </a>
  </p>
</div>

---

### 📊 Engineering Impact & Scale

<div align="center">
  <table>
    <tr>
      <td align="center" width="25%">
        <h2>⏳ 12+</h2>
        <sub><b>Years Engineering Excellence</b></sub>
      </td>
      <td align="center" width="25%">
        <h2>📱 30+</h2>
        <sub><b>Production Apps Shipped</b></sub>
      </td>
      <td align="center" width="25%">
        <h2>👥 1M+</h2>
        <sub><b>Active Users Across GCC & Africa</b></sub>
      </td>
      <td align="center" width="25%">
        <h2>⚡ 99.9%</h2>
        <sub><b>Uptime Distributed Systems</b></sub>
      </td>
    </tr>
  </table>
</div>

---

### 👨‍💻 Executive Developer Cockpit

```kotlin
data class SoftwareArchitect(
    val name: String = "Irfan Ullah (Ibrahim Khan)",
    val location: String = "Kuwait City, Kuwait 🇰🇼",
    val title: String = "Project Manager & Principal Kotlin Multiplatform Architect",
    val experienceYears: Int = 12,
    val coreSpecialties: List<String> = listOf(
        "Kotlin Multiplatform (KMP & CMP)",
        "Distributed High-Concurrency Backends (Ktor, Node.js)",
        "Real-Time Telemetry, OSRM Routing & Geofencing Engines",
        "Cross-Platform Engineering Leadership & Architecture"
    ),
    val currentMission: String = "Leading Faras Captain Kenya & Winch Roadside Assistance Platforms",
    val philosophy: String = "Clean Architecture, Type-Safety, Zero-Bloat, and Butter-Smooth 60FPS UI"
)
```

---

### 🌟 Featured Enterprise Flagships

<table>
  <tr>
    <td width="50%" valign="top">
      <h3 align="center">🚗 Faras Captain (Kenya)</h3>
      <p align="center">
        <b>Role:</b> Project Manager & Lead Mobile Architect<br/>
        <b>Stack:</b> Kotlin Multiplatform, Compose, Native iOS, WebSockets<br/>
        Large-scale mobility & driver dispatch platform serving tens of thousands of active drivers across East Africa.
      </p>
      <p align="center">
        <a href="https://play.google.com/store/apps/details?id=com.faras.driver" target="_blank">
          <img src="https://img.shields.io/badge/Google_Play-Faras_Captain-34A853?style=for-the-badge&logo=google-play&logoColor=white" alt="Google Play"/>
        </a>
      </p>
    </td>
    <td width="50%" valign="top">
      <h3 align="center">🛞 Winch Roadside Assistance</h3>
      <p align="center">
        <b>Role:</b> Principal Architect & Full-Stack Lead<br/>
        <b>Stack:</b> KMP, Ktor Gateway, PostgreSQL, React Admin Portal, OSRM<br/>
        Next-gen roadside assistance dispatch ecosystem with live high-precision GPS tracking, offline resilience, and dynamic KYC validation.
      </p>
      <p align="center">
        <img src="https://img.shields.io/badge/Status-Active_Production-00CEC9?style=for-the-badge&logo=fastapi&logoColor=white" alt="Active Production"/>
      </p>
    </td>
  </tr>
</table>

---

### 🛠️ Interactive Tech Stack Matrix

<div align="center">

#### 📱 Languages & Mobile Platforms
<p align="center">
  <img src="https://skillicons.dev/icons?i=kotlin,swift,flutter,android,apple,java,dart&theme=dark" alt="Mobile Stack"/>
</p>

#### ⚡ Backend, Cloud & Databases
<p align="center">
  <img src="https://skillicons.dev/icons?i=ktor,nodejs,typescript,javascript,postgres,redis,mongodb,mysql,docker,nginx,aws&theme=dark" alt="Backend Stack"/>
</p>

#### 🌐 Web, Tooling & DevOps
<p align="center">
  <img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,git,github,githubactions,figma,postman,gradle&theme=dark" alt="DevOps Stack"/>
</p>

</div>

---

### 🏛️ Core Architectural Pillars

<table>
  <tr>
    <td width="50%">
      <h4>📱 Multiplatform Excellence</h4>
      <ul>
        <li><b>100% Shared Business Logic:</b> KMP networking, repositories, and ViewModels.</li>
        <li><b>Native Fidelity:</b> Jetpack Compose on Android & Compose Multiplatform / SwiftUI on iOS.</li>
        <li><b>Robust DI & Flow:</b> Koin dependency injection and reactive Kotlin Coroutines & StateFlow.</li>
      </ul>
    </td>
    <td width="50%">
      <h4>⚡ Real-Time & Distributed Systems</h4>
      <ul>
        <li><b>Sub-Second Telemetry:</b> Bidirectional WebSockets for live driver tracking.</li>
        <li><b>Road Network Routing:</b> Self-hosted OSRM turn-by-turn routing & dynamic ETA recalculation.</li>
        <li><b>High-Concurrency:</b> Non-blocking Ktor coroutines with PostgreSQL & Redis caching.</li>
      </ul>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <h4>🛡️ Mission-Critical Reliability</h4>
      <ul>
        <li><b>Offline First:</b> Local SQL persistence & automated network retry sync.</li>
        <li><b>Strict Compliance:</b> Dynamic per-document verification & Single-Device session enforcement.</li>
        <li><b>End-to-End Security:</b> JWT bearer rotation, role-based access control, and audited security rules.</li>
      </ul>
    </td>
    <td width="50%">
      <h4>🎯 Agile Engineering Leadership</h4>
      <ul>
        <li><b>Team Empowerment:</b> Sprint velocity optimization, code quality gating, and mentorship.</li>
        <li><b>CI/CD Pipelines:</b> Automated GitHub Actions builds, Gradle optimization, and test automation.</li>
        <li><b>Product Roadmapping:</b> Translating complex business requirements into elegant software.</li>
      </ul>
    </td>
  </tr>
</table>

---

### 🧭 2026 Technical Radar

- 🟢 **Active Focus:** Advanced Kotlin Multiplatform (KMP 2.0+), Compose Multiplatform for iOS 18+
- 🟡 **Scaling:** High-throughput event-driven microservices with Ktor, Redis Streams & WebSockets
- 🟣 **Exploring:** Local AI inference models and on-device intelligent routing heuristics
- 🤝 **Collaborations:** Open to enterprise advisory, technical leadership, and scalable KMP solutions

---

### 📊 GitHub Activity & Real-Time Analytics

<div align="center">
  <table border="0">
    <tr>
      <td width="50%">
        <img width="100%" src="https://github-readme-stats-6v5kbwzd2-itsmeirfanullah-9109s-projects.vercel.app/api?username=iukust3&show_icons=true&include_all_commits=true&count_private=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=6C5CE7&icon_color=00CEC9&text_color=DFE6E9&v=7" alt="GitHub Stats"/>
      </td>
      <td width="50%">
        <img width="100%" src="https://github-readme-stats-6v5kbwzd2-itsmeirfanullah-9109s-projects.vercel.app/api/top-langs/?username=iukust3&layout=compact&count_private=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=6C5CE7&text_color=DFE6E9&v=7" alt="Top Languages"/>
      </td>
    </tr>
  </table>

  <p align="center">
    <img src="https://github-readme-streak-stats.herokuapp.com/?user=iukust3&theme=tokyonight&hide_border=true&background=0D1117&stroke=6C5CE7&ring=6C5CE7&fire=00CEC9&currStreakLabel=6C5CE7&v=4" alt="GitHub Streak"/>
  </p>

  <p align="center">
    <img src="https://github-profile-trophy.vercel.app/?username=iukust3&theme=tokyonight&no-frame=true&row=1&column=7&margin-w=15" alt="Trophies"/>
  </p>
</div>

---

### 🐍 Contribution Activity

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/iukust3/iukust3/output/github-contribution-grid-snake-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/iukust3/iukust3/output/github-contribution-grid-snake.svg">
    <img alt="GitHub Contribution Snake" src="https://raw.githubusercontent.com/iukust3/iukust3/output/github-contribution-grid-snake.svg">
  </picture>
</div>

---

### 🤝 Connect & Support

<div align="center">
  <p>
    <b>Let's build something extraordinary together!</b><br/>
    Open for consulting, enterprise architectural advisory, and high-impact partnerships.
  </p>

  <p>
    <a href="mailto:itsme.irfanullah@gmail.com">
      <img src="https://img.shields.io/badge/Get_In_Touch-itsme.irfanullah%40gmail.com-6C5CE7?style=for-the-badge&logo=gmail&logoColor=white" alt="Contact"/>
    </a>
    <a href="https://www.buymeacoffee.com/iukust3" target="_blank">
      <img src="https://img.shields.io/badge/Support_My_Work-Buy_Me_A_Coffee-FFDD00?style=for-the-badge&logo=buy-me-a-coffee&logoColor=black" alt="Buy Me A Coffee"/>
    </a>
  </p>

  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://capsule-render.vercel.app/api?type=waving&color=0:0D1117,25:1E1F3B,55:6C5CE7,80:8E44AD,100:00CEC9&height=100&section=footer">
    <source media="(prefers-color-scheme: light)" srcset="https://capsule-render.vercel.app/api?type=waving&color=0:F0EEFF,25:DDD6FE,55:C4B5FD,80:A78BFA,100:8B5CF6&height=100&section=footer">
    <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0D1117,25:1E1F3B,55:6C5CE7,80:8E44AD,100:00CEC9&height=100&section=footer" width="100%"/>
  </picture>
</div>
